...
found:
  ...
  // Syscall-2 of Spec-1 of MP2 : initialise proc struct values.
  p->currTicks = 0;
  p->alarmCache = 0;
  p->turnAlarmOn = 0;
  p->maxTicks = 0;
  ...
  // MLFQ of Spec-2 of MP2
  p->currPQ = 0;
  p->PQpos = lastidxinPQ[0]++; // PQ 0 by default.
  p->ticksElapsed = 0;
  p->waitTicks = 0;

  // Mini Project - 3 xv6 Spec-1 for modified PBS
  p->sPriority = 50; // Given, the default static priority of a process is 50.
  p->rTime = 0;
  p->sTime = 0;
  p->wTime = 0;
  p->RBI = 25; // Given, the default RBI of a process is 25.
  p->dPriority = 75; // By default, based on the DP = min(SP + RBI, 100) calculation, DP is 75.
  p->numScheds = 0;
...
#elif defined(PBS) // Mini Project - 3 xv6 Modified PBS
    for (;;)
  {
    // Avoid deadlock by ensuring that devices can interrupt.
    intr_on();
    ...
    struct proc* currProcess = 0;
    // Figure out which process to run. Use the given tie-breaker.
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      // Update dynamic priority of this process first, and then check.
      int newRBIcalc = ((3*p->rTime-p->sTime-p->wTime) * 50)/(p->rTime+p->sTime+p->wTime+1);
      p->RBI = newRBIcalc > 0 ? newRBIcalc : 0;
      p->dPriority = p->sPriority + p->RBI < 100 ? p->sPriority + p->RBI : 100;

      // Now see if you can choose this process.
      if (p->state == RUNNABLE && (!currProcess || ((p->dPriority < currProcess->dPriority) || (p->dPriority == currProcess->dPriority && p->numScheds < currProcess->numScheds) || (p->dPriority == currProcess->dPriority && p->numScheds == currProcess->numScheds && p->ctime < currProcess->ctime)))) {
          if(currProcess) release(&currProcess->lock);
          currProcess = p;
          continue;
      }
    ...
    if(currProcess){ // Same code from round robin, to switch to the process.
        ...
        // before jumping back to us.
        #ifdef PBSSTAT
        printf("%d %d %d %d\n", ticks, currProcess->pid, currProcess->sPriority, currProcess->dPriority);
        #endif

        currProcess->numScheds++;
        currProcess->sTime = 0, currProcess->rTime = 0;
//        currProcess->wTime = 0;
        currProcess->state = RUNNING;
        c->proc = currProcess;
        swtch(&c->context, &currProcess->context);
...
#elif defined(PBS)
      printf("\nNUM TIMES PROCESS HAS BEEN SCHEDULED: %d CURRENT STATIC PRIORITY: %d CURRENT RUNNING TIME: %d CURRENT SLEEPING TIME: %d CURRENT WAITING TIME: %d CURRENT RBI: %d CURRENT DYNAMIC PRIORITY: %d", p->numScheds, p->sPriority, p->rTime, p->sTime, p->wTime, p->RBI, p->dPriority);
#endif
    printf("\n");
  }
}
...
// Modified PBS of Mini Project - 3 of Spec-1
int modifyPriority(int pid, int sPriority){
    int oldStaticPriority = -1;
    for (struct proc* p = proc; oldStaticPriority == -1 && p < &proc[NPROC]; p++) // This is where we actually do stuff! We can choose which process to run here. This, we will do using FCFS. THIS LOOP IS ONLY TO CHOOSE APPROPRIATE PROCESS!
    {
        acquire(&p->lock);
        if(p->pid == pid){
            oldStaticPriority = p->sPriority;
            p->sPriority = sPriority;
            p->RBI = 25; // Reset the process's recent behaviour index, as given.
        }
        release(&p->lock);
    }
    yield(); // Yield to find the best process to run at this instant.
    return oldStaticPriority;
}